...
Users can forward health‑related messages to a Telegram bot and receive a quick, concise explanation based on trusted medical information instead of random internet claims.
...
- Returns short, human‑readable replies explaining why a claim looks reliable or suspicious.
...
- Simple Python/Flask codebase that can be extended with ML/NLP models later.
...
3. The API loads `trusted_data.json`, searches for relevant entries, and returns:
   ...
   - A short explanation or reference.
...
- Flask (REST API)  
- Telegram Bot API  
- JSON (`trusted_data.json` as a simple knowledge base).
